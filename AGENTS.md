...
- [0025](doc/decisions/0025-persist-control-parameters-flash.md) — Persist control parameters to flash: store the postcard-encoded `ControlSystemParameters` (ADR 0024) in a reserved 1–2 page region of nRF52833 internal flash via a log-structured store (`sequential-storage` over embassy-nrf `Nvmc`), so live-tuned gains survive a power cycle without a rebuild/reflash. Explicit-commit writes only (GS "Save gains to flash"), disarmed-only, off the control path (page erase blocks CPU ~85 ms). Boot reads the latest valid record into the `control_system_parameter_update` Watch, falling back to `::default()` on empty/CRC-fail/version-mismatch (fail-safe schema evolution via a version tag, no migration). Flash region reserved in `memory.x`, board-specific per ADR 0010. Deliberate stepping stone onto the internal-flash stack for a later OTA effort, minus the bootloader. (Proposed) (2026-08-14)
