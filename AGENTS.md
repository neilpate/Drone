...
- A telemetry-log analyser already exists: [crates/groundstation/src/bin/analyze.rs](crates/groundstation/src/bin/analyze.rs). Run it with `cargo run --bin analyze -- <path-to-telemetry.tsv>` for a diagnostic report (phase segmentation, per-channel stats over the active-throttle window, attitude drift, motor asymmetry, demand saturation).
- Use it to analyse a `telemetry-*.tsv` capture (in [crates/groundstation/](crates/groundstation/)) rather than hand-rolling ad-hoc parsing. The parser is header-driven, so it tolerates column reordering/additions.
...
- [0024](doc/decisions/0024-control-law-angle-mode-pd.md) — Control law: single-loop PD per axis (not cascaded), angle mode for roll/pitch (stick = desired angle vs the ADR 0022 estimate), rate mode for yaw (stick = desired rate vs gyro Z). D on the measured gyro rate, not the error (no derivative kick). Setpoint scaling via MAX_TILT_ANGLE / MAX_YAW_RATE; throttle passes through; per-axis output clamped. Integral deferred (PD first; anti-windup + reset-on-disarm named for when I lands). Pure `controller` logic in firmware-drone-core, driven by a single-publisher stage owning a `ControllerDemand` Watch; the supervisor (ADR 0017) mixes it in Armed and overrides it in Degraded/Fault. Gains/maxima are tuning, not decided here. Fills the `06-control.md` slot. (Proposed) (2026-07-22)
- [0025](doc/decisions/0025-persist-control-parameters-flash.md) — Persist control parameters to flash: store the postcard-encoded `ControlSystemParameters` (ADR 0024) in a reserved 2-page (8 KiB) region of nRF52833 internal flash via a log-structured store (`sequential-storage` over embassy-nrf `Nvmc`), so live-tuned gains survive a power cycle without a rebuild/reflash. Explicit-commit writes only (GS "Save gains to flash"), disarmed-only, off the control path (page erase blocks CPU ~85 ms). Boot reads the latest valid record into the `control_system_parameter_update` Watch, falling back to `::default()` on empty/CRC-fail/version-mismatch (fail-safe schema evolution via a version tag, no migration). Flash region reserved in `memory.x`, board-specific per ADR 0010. Deliberate stepping stone onto the internal-flash stack for a later OTA effort, minus the bootloader. (Accepted, implemented 2026-09-19) (2026-08-14)
- [0026](doc/decisions/0026-phase4-custom-pcba-nrf5340.md) — Phase 4 custom PCBA: build the flight controller around a pre-certified nRF5340 module on a hand-designed KiCad carrier (not bare silicon — module supplies antenna/matching/32 MHz HFXO/DC-DC/cert, cutting first-board RF risk; stays in nRF family; no 32.768 kHz LFXO fitted — LFCLK runs off the internal RC, P0.00/P0.01 reserved). Assembly is TURNKEY PCBA (partial: module + maybe IMU sourced externally, vendor sources the rest), not hand-soldered — de-risks the QFN IMU + module joints. Because a reflow house handles LGA, hand-solderability is NOT a module criterion; criteria are programmable-over-SWD + antenna + footprint/stock; candidates Raytac MDBT53 / Fanstel / u-blox NORA-B1 / Ezurio BL5340 / Insight SiP all in contention (LGA back on the table); exact part deferred, PCBA vendor is Aisler (chosen over JLCPCB/PCBWay/Eurocircuits for easier module sourcing and UK lead time). No onboard DAPLink: native nRF5340 USB-CDC for the PC/telemetry link (collapses ADR 0018's USB→interface→UART path to USB→nRF5340) + an SWD header for an external probe (J-Link / CMSIS-DAP / Pi Debug Probe / spare micro:bit or nRF5340-DK, all probe-rs). Carries over ICM-42688-P on SPI (ADR 0003); commits to DShot for the 4 motors; regulates from 4S (ADR 0019) with a Vbat divider to ADC. nRF5340 dual-core: 802.15.4 (ADR 0014) on the network core, app logic on the app core; BSP (ADR 0010) gains a new feature-selected board, most task code expected to port. Extends ADR 0002 (micro:bit Phases 1–3 → nRF5340 Phases 4–5) without superseding it; micro:bit stays the tuning platform in parallel. First electrical design in the repo; source + fab outputs under `hardware/electrical/`. (Proposed) (2026-08-19, amended 2026-08-31 — PCBA vendor)
- [0027](doc/decisions/0027-split-telemetry-high-low-rate.md) — Split telemetry into high-rate and low-rate frames: the drone→remote reply becomes a tagged `TelemetryFrame { HighRate, LowRate }` enum because the single `Telemetry` frame hit the 802.15.4 ceiling (measured 123 B of ~125 usable; ESC `DShotTelemetry` +31 B = 154 B, over). HighRate every round-trip (~100 Hz): seq, raw IMU accel+gyro, filtered gyro, attitude, pilot_command (also the RTT echo, must stay high-rate), controller_demand, motor_command, drone_state — accel sent once, de-bloating the old two-full-`Sensors` frame. LowRate every Nth round-trip (~10 Hz): seq, cpu_load, control_mode, chip temp, ESC `DShotTelemetry` (all 4 motors), and the drone's active `ControlSystemParameters` echoed for ground-truth logging (closes the GS-logs-its-own-copy divergence when an update drops or the drone runs boot defaults). Aggregator stays the single publisher (ADR 0020) and owns scheduling; GS carries last-known low-rate state and merges into each logged row (TSV header unchanged). Growth path: interleave further low-rate variants rather than inflating one frame. Extends ADR 0018/0020, bounded by ADR 0014. (Proposed) (2026-08-22)
- [0028](doc/decisions/0028-fabricate-v2-board-pcbway.md) — Fabricate the v2 flight-controller board: executes ADR 0026's platform decision by committing to build the `drone_fc` **v2** KiCad board (`hardware/electrical/v2/`; Raytac MDBT53 nRF5340, ICM-42688-P, 8-pin JST-SH 4-in-1 ESC, LMR50410 buck + 3.3 V LDO from 4S, native USB, SWD). v3 was unfinished and deleted. Vendor is **PCBWay turnkey** — **supersedes ADR 0026's Aisler choice** (PCBWay quoted the full turnkey build including sourcing the module, removing the consign logistics that drove the Aisler preference). Order: 5 bare boards (fab MOQ), 2 assembled (1 flight + 1 spare), 8 Raytac modules (module MOQ) — leaves 3 spare boards / 6 spare modules. Cost recorded (figures pending final quote). The spare assembled board can double as the radio remote (identical nRF5340 = ideal link symmetry) but NOT by direct joystick wiring: the four ESC pins (P0.03/P0.16/P0.17/P0.22) are not SAADC-capable and the only broken-out AINs (P0.04/P0.27 analog, P0.07/P0.28 digital) are all used — a remote needs an external I²C/SPI ADC on the ESC connector, no respin. Pre-fab risk: verify the reversed 8-pin JST-SH ESC pinout (pin 8 = VBAT up to 16.8 V onto a GPIO is destructive). (Proposed) (2026-09-03)
- [0029](doc/decisions/0029-multi-board-firmware-shared-library.md) — Multi-board firmware structure: avoid copy-pasting `firmware-drone` per board by extracting a `firmware-drone-shared` library (board-agnostic Embassy tasks, signal statics, peripheral traits) that layers on the still-pure host-tested `firmware-drone-core` (ADR 0015 boundary kept; merge rejected). Revises ADR 0010's "board seam inside the binary": the seam becomes a library boundary, each binary keeps its own feature-selected `board` module but the logic consuming it lives one crate up. Two task classes: Group A (no hardware — supervisor, attitude estimator, control system, telemetry/sensors aggregators) move to `shared` with their `#[embassy_executor::task]` intact; Group B (touches hardware — imu, motor_controller, remote_link, status_led, temperature, esc_telemetry, config_manager) become generic `async fn` bodies in `shared` bounded by seven peripheral traits (`MotorDriver`/`ImuReader`/`RadioLink`/`StatusIndicator`/`TemperatureSource`/`EscTelemetrySource`/`ConfigStore`, defined in `shared`), each board binary supplying a 3-line `#[embassy_executor::task]` shim — the standard Embassy workaround for non-generic tasks, generalising ADR 0010's name-stable `board::X` trick across crates. Load-bearing rule: `firmware-drone-shared` must NOT depend on `embassy-nrf` (chip feature is mutually exclusive per build) — only chip-agnostic `embassy-executor/time/sync` + `firmware-types`/`firmware-drone-core`/`defmt`; `embassy_nrf::init`, time driver, InterruptExecutor + `#[interrupt]`, `memory.x`, `build.rs`, `.cargo/config.toml` stay per-binary. Driven by the nRF5340 being a different target triple (`thumbv8m.main` vs `thumbv7em`) AND dual-core (two binaries) — so it can't be just another `board-*` feature of one binary. Binaries renamed symmetric: `firmware-drone` → `firmware-drone-microbit`, plus `firmware-drone-nrf5340-app` + `firmware-drone-nrf5340-net` (extends/supersedes ADR 0009's `firmware-pcba` placeholder). nRF5340 net core (ADR 0014/0026 radio-on-net-core) is a DUMB raw-802.15.4-PHY-frame byte pipe over IPC — all postcard/COBS decode + `firmware-types` stay on the app core, so `RadioLink` is one symmetric interface over on-chip radio (micro:bit) and IPC endpoint (nRF5340). `StatusIndicator` reshaped intent-first (`set_state(SystemStatus)`, not `on()`/`off()`). `spawn_common(&spawner)` in `shared` spawns Group-A on both boards so the list can't diverge. Two-phase migration: Phase 1 NOW (create `shared`, move Group-A + signals, add `spawn_common`, rename to `-microbit`; pure refactor, single consumer) — Phase 2 during nRF5340 bring-up (introduce traits, convert Group-B bodies, driven by the real 2nd board). New nRF5340 binaries (different triple) will need the `groundstation` workspace-exclude / verify-separately treatment. Scope caveats: the REMOTE stays micro:bit-only (out of scope, keeps its single-board structure + own radio code; `RadioLink` is drone-internal, not a shared drone/remote trait); board-unique tasks are allowed in principle though none currently anticipated (the two boards are expected task-symmetric); ADR 0010's `board-*` Cargo features are DROPPED (board = crate identity now). nRF5340 two-binary split is NOT two independent programs: app core brings up net core (SPU grants RADIO/shared-RAM/GPIO access + releases net-core reset), the IPC shared-RAM region is a cross-binary linker contract (both `memory.x` must agree), app core is secure-world-only (`nrf5340-app-s`, no TrustZone split). Top Phase-2 de-risk = verify the inter-core IPC transport exists/is ergonomic in embassy-nrf BEFORE committing to the dumb-pipe `RadioLink`-over-IPC design (Nordic's path is IPC-peripheral + hand-rolled shared-RAM ring; fallback is pushing more protocol onto the net core). Also §11: widen the core/task split opportunistically during the move (candidates, not Phase-1 gate) — scan found strong pure-logic extractions to `firmware-drone-core`: ICM-42688 raw-byte→ImuData parser (highest payoff, both boards share the IMU per ADR 0003), status-LED `DroneState`→pattern table, IMU calibration averaging, CPU-load formula; radio dispatch/ESC frame scan/telemetry assembly/sensor packing left in tasks (too signal- or `.await`-coupled); supervisor/attitude/control/temperature/DShot already delegate to core. (Accepted) (2026-09-20)
