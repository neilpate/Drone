...
Roll pairs with the gyro's X (forward) axis, pitch with Y (right), yaw rate with Z (down). The IMU is mounted **axis-aligned to the body FRD frame**, confirmed by a static tilt test (level `a_z = -1 g`; nose-up gives `+a_x`; roll-right gives `-a_y`), so the filter reads the chip axes directly as FRD with no remap. Re-run that tilt test if the sensor is ever remounted. Angles are held internally in **degrees**, not radians. The gyro reads deg/s, so the gyro-prediction step (`angle + rate * dt`) integrates with no unit conversion; only the accelerometer terms convert, since `atan2` returns radians (`.to_degrees()` applied at that step). Radians reappear only where downstream trigonometry would need them, and nothing in this filter or the angle-mode control path does. Degrees are also the unit humans reason in — a 30° bank or a 10° nose-up pitch is immediately meaningful where 0.52 rad or 0.17 rad is not — so telemetry surfaces the attitude in degrees directly (as does `defmt`), with no conversion at the boundary.
