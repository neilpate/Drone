...
- [imu-vibration-and-bench-psu-power.md](imu-vibration-and-bench-psu-power.md) — the closed-loop controller looked wildly unstable on the bench, but it was the test rig, not the PID: a hard-mounted IMU turned motor vibration into gyro noise that the D-term amplified, and a bench PSU could not power the motors (foldback brownout + it cannot sink regen current, so throttle-down trips OVP and cycles the output relay). Open-loop mode isolated the plant; soft-mounting and a battery are the fixes. When a loop looks unstable, suspect the rig before the gains. (2026-07-22)
