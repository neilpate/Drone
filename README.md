...
**Phase 2 (advanced prototyping) in progress.** The whole stack runs on hardware. The drone has made its **first free flights** — brief untethered, self-levelling hops on its own battery, with telemetry over the two-hop radio link and gains **tunable live from the ground station** — and the motor drivetrain has since gone digital: outgoing **DShot300** replaced analog PWM and, in the switch, cured a front-left thrust asymmetry that had been fought as a weak motor but was really uncalibrated per-channel PWM endpoints. Retuned on the now-symmetric plant, it hovers under closed-loop control on DShot, and each ESC now reports **serial telemetry** — per-motor RPM, pack voltage and temperature — back over the link.

- **Command + telemetry round trip, 100 Hz, ~25–30 ms.** The `groundstation` app (binary `gs`) sends a four-axis pilot command — throttle plus roll/pitch/yaw — from on-screen sliders or a PlayStation gamepad, over USB-CDC to a remote micro:bit, which relays it to the drone micro:bit over IEEE 802.15.4 ([ADR 0014](doc/decisions/0014-radio-protocol-ieee802154.md)). Telemetry returns the same path — a tagged frame **split into a fast flight-data stream and a slower housekeeping stream** ([ADR 0027](doc/decisions/0027-split-telemetry-high-low-rate.md)) so the ESC data fits under the packet ceiling — postcard + COBS framed, with every wire buffer sized at compile time from the shared types.
...
- **Actuation + ESC telemetry.** Outgoing **DShot300** drives a Sequre Blueson A1 (AM32) 4-in-1 ESC ([parts list](hardware/electrical/parts-list.md)) — a looped 16-bit frame per motor played from PWM0 + EasyDMA, with a host-tested frame encoder in `firmware-drone-core`. Sending identical digital commands (rather than per-channel-calibrated analog pulses) eliminated the front-left thrust asymmetry. Each ESC's **KISS serial telemetry** — temperature, pack voltage and per-motor RPM — is read back over an idle-framed UART, requested one motor at a time round-robin and CRC-validated; it rides the low-rate telemetry frame and is plotted and logged on the ground station (the ESC's uncalibrated current shunt is discarded).
