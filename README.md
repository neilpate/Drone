...
**Next (to finish Phase 2):** repair the cracked struts to restore a rigid airframe, and resolve the front-left (M4) thrust asymmetry — a fresh set of props didn't clear it, so it's a motor/ESC/mount issue — then return to flight-tuning the roll/pitch gains toward a sustained hover. Beyond that, the outstanding Phase 2 milestones: DShot in place of PWM, a firmware power-limit mode, and the netted test enclosure ([07-safety.md](doc/07-safety.md)).
