...
**Phase 2 (advanced prototyping) in progress.** The whole stack runs on hardware: a PC ground station drives the drone over a two-hop link and gets live telemetry back, the drone flies a **closed, self-levelling control loop** on the bench with all four motors, and the PD gains are **tunable live from the ground station** (not yet flight-tuned or airborne).
...
- **Sensing + estimation.** The ICM-42688-P IMU ([ADR 0003](doc/decisions/0003-imu-icm42688-spi.md)) is read over SPI at 1 kHz; a complementary filter ([ADR 0022](doc/decisions/0022-attitude-estimation-complementary-filter.md)) fuses it into a roll/pitch attitude estimate, with a startup accelerometer calibration that zeroes the mount offset at the source (a level craft reads ~0).
- **Closed-loop control.** A single-loop **angle-mode PD** controller ([ADR 0024](doc/decisions/0024-control-law-angle-mode-pd.md)) turns the attitude estimate and pilot command into a mixed four-motor demand; the supervisor mixes it when Armed and overrides it in failsafe. A gamepad toggle switches **Stabilized** (closed-loop) versus **Manual** (raw passthrough) for diagnostics. A first-order gyro **low-pass filter** feeds the D term to keep motor-vibration noise out of the demand, and every gain and limit is a runtime `ControlSystemParameters` message the ground station can push live from a gains panel — no reflash to tune.
...
The ground station plots and logs every signal live — pilot command, six-axis IMU, attitude estimate, controller demand and the four mixed motor outputs — times the round trip, and pushes PD gains to the drone live from a gains panel:
...
**Next (to finish Phase 2):** flight-tune the PD gains now the enabling pieces are in place (soft-mounted IMU, gyro low-pass filter, live gain tuning) — the remaining bench-rig fix is battery power, since a PSU browns out under motor load and cannot sink regenerative current ([imu-vibration-and-bench-psu-power.md](doc/learning/imu-vibration-and-bench-psu-power.md)) — then the outstanding Phase 2 milestones: DShot in place of PWM, a firmware power-limit mode, and the netted test enclosure ([07-safety.md](doc/07-safety.md)) before the first props-on full-power run.
